...
function updateShareStats() {
    ...
    async.waterfall([
        ...
        function (height, callback) {
            let locTime = Date.now() - (hashrate_avg_min*60*1000);
            let identifierTime = Date.now() - (2*hashrate_avg_min*60*1000);
            ...
            async.doUntil(function (callback_until) {
                ...
                minerList.forEach(function (miner) {
                    ...
                    console.log("Adding: " + miner + " as an active miner to the cache.");
                    if (miner.indexOf('_') <= -1) return;
