...
// time of last scan for dead worker_name
let walletWorkerScanTime = {};
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (!(wallet_key in walletAcc)) {
        ...
        walletWorkerScanTime[wallet_key] = time_now;
    }
    ...
    if (time_now - walletWorkerScanTime[wallet_key] > 60*1000) {
        //console.log("!!! " + wallet_key + ": scanning for old worker names");
        let wallet = walletAcc[wallet_key];
        for (let worker_name2 in wallet) {
            let worker2 = wallet[worker_name2];
            if (time_now - worker2["time"] > 60*1000) {
                let acc2 = worker2["acc"];
                if (acc2 != 0) {
                    let height2 = worker2["height"];
                    //console.log("!!! " + wallet_key + " / " + worker_name2  + ": storing old worker share " + height2 + " " + worker2["difficulty"] + " " + time_now + " " + acc2);
                    global.database.storeShare(height2, global.protos.Share.encode({
                        shares: acc2,
                        paymentAddress: miner.address,
                        paymentID: miner.paymentID,
                        foundBlock: blockCandidate,
                        ...
                        poolType: miner.poolTypeEnum,
                        poolID: global.config.pool_id,
                        blockDiff: worker2["difficulty"],
                        bitcoin: miner.bitcoin,
                        blockHeight: height2,
                        timestamp: time_now,
                        identifier: worker_name2
                    }));
                }
                //console.log("!!! " + wallet_key + ": removing old worker " + worker_name2);
                if (worker_name2 !== "all_other_workers") -- walletWorkerCount[wallet_key];
                delete wallet[worker_name2];
            }
        }
        walletWorkerScanTime[wallet_key] = time_now;
    }
