...
function Database(){
    ...
    this.getOldestLockedBlock = function(){
        ...
        */
        ...
        let txn = this.env.beginTxn({readOnly: true});
        let cursor = new this.lmdb.Cursor(txn, this.blockDB);
        let highestBlockKey = null;
        let highestBlock = null;
        for (let found = cursor.goToFirst(); found; found = cursor.goToNext()) {
            cursor.getCurrentBinary(function(key, data){  // jshint ignore:line
                let blockData = global.protos.Block.decode(data);
                if (blockData.unlocked === false){
                    if (highestBlock === null || highestBlockKey >= key) {
                        if (highestBlock !== null) {
                            console.error("Found unordered blocks with " + highestBlockKey.toString() + " and " + key.toString() + " heights");
                        }
        ...
        async.waterfall([
            ...
            function (lastBlock, difficulty, callback) {
                ...
                range.range(lastBlock-1, 0, -1).forEach(function (blockID) {
                    ...
                    cursor.close();
                    txn.abort();
                    if (pplnsCurrFound) pplnsFound = true;
                });
